...
// Page Component
export default function Mockup2InvoicePage() {
  const [isFormOpen, setIsFormOpen] = React.useState(true);
  const [data, setData] = React.useState<InvoiceData>({
    invoiceNo: '0852/SMFI/II/26',
    amountInWords: 'Delapan  Belas  Juta  Lima  Ratus  Ribu  Rupiah',
    row1: 'Pelunasan Pasang 1 (satu) Unit Filter air Mizu Firuta MF 300 1T + 1 (satu) Bh Spare',
    row2: 'part WRV & Instalasi Pipa Induk Oleh Pihak Apartment',
    row3: 'Ket : 5 (Lima) Tahun Garansi Tabung Filter air MF 300 1T',
    row4: '1 (Satu) Tahun Garansi Spare part Filter air Mizu Firuta MF 300 1T',
    row5: '3 (Tiga) Kali Free Maintenance untuk Tahun Pertama Periode 4 Bulan Sekali',
    row6: 'Kunjungan Service, Ganti Media Carbon Powder & Cloth Filter',
    row7: 'Transfer Rekening BCA : 6275256998 - CV Surya Mizu Firuta Indowater',
    amountNumeric: '18.500.000,-',
  ...
  const processExport = async (dataToExport: InvoiceData, filename: string) => {
    ...
    try {
      ...
      const response = await fetch('/api/invoice-pdf2', {
        method: 'POST',
        headers: {
